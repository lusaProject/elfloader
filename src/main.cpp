...
static bool read_file(const char *fname, void **buf, size_t *len)
{
    ...
    tmp_buf = (char *)malloc(st.st_size);
    if (tmp_buf == NULL)
    {
...
void PluginCallBack(void *context, void *data, long long datalen, int status)
{
    printf("run in PluginCallBack:%s", (char *)data);
}
...
int main(int argc, char *argv[])
{
    ...
    // run_elf_module(m, argv[2],NULL,NULL,0,PluginCallBack);

    so_elf_module(m, argv[2], "test", 8);
