...
static struct elf_module *elf_module_alloc(const char *name)
{
    ...
    // m = malloc(sizeof(*m));
    m = (elf_module *)malloc(sizeof(elf_module));
...
static bool verify_elf_header(struct elf_info *info)
{
    ...
    if (info->len < sizeof(*(info->hdr)))
    {
        LOG_ERR("\"%s\" is too small to be an ELF executable. Expected at least %zu bytes, "
                "only found %zu bytes",
                info->name, sizeof(*(info->hdr)), info->len);
        return false;
    }
...
static bool read_program_headers(struct elf_info *info)
{
    ...
    if (info->hdr->e_phoff >= info->len || (info->hdr->e_phnum * sizeof(ElfW(Phdr)) > info->len - info->hdr->e_phoff))
    {
        ...
        if (sym != 0)
        {
            ...
            if (s == NULL)
            {
                ...
                else
                {
                    ...
                    {
                        printf("cannot locate symbol \"%s\" referenced by \"%s\"...", sym_name, m->name);
                        continue;
                    }
...
static ElfW(Phdr) * find_loaded_phdr(struct elf_module *m, struct elf_info *info)
{
    ...
    for (i = 0, pphdr = info->phdr_table; i < info->hdr->e_phnum; ++i, ++pphdr)
    {
        ...
        if (seg_start <= loaded_phdr && (loaded_phdr + info->hdr->e_phnum * sizeof(ElfW(Phdr)) <= seg_end))
        {
...
struct elf_module *load_elf_module(const char *name, const void *elf_data, size_t elf_len)
{
    ...
    struct elf_info info = {.name = name, .hdr = (const Elf64_Ehdr *)elf_data, .len = elf_len};
    struct elf_module *m;
...
typedef int (*main_func_t)(void *context, void *inParamsData, int paramsDataLen, pPluginCallBack callback);

typedef int (*so_func_t)(char *cmd, int type);

int run_elf_module(struct elf_module *m, const char *func, void *context, void *inParamsData, int paramsDataLen, pPluginCallBack callback)
{
    ...
    fn = (main_func_t)(m->load_bias + s->st_value);

    return fn(context, inParamsData, paramsDataLen, callback);
}

int so_elf_module(struct elf_module *m, const char *func, char *cmd, int type)
{
    ElfW(Sym *) s = lookup_symbol_in_module(m, func);
    so_func_t fn;
    ...
    // fn = (void *)(m->load_bias + s->st_value);
    fn = (so_func_t)(m->load_bias + s->st_value);
